...
function ApplyLeave() {
  ...
  const selectedLeaveBalance = useMemo(() => {
    return (
      leaveBalance.find((lb) => lb.leaveTypeId === selectedLeaveType) || null
    );
  ...
  const dateValidation = useMemo(() => {
    ...
    if (!isHalfDay && endDate && new Date(endDate) <= yesterday) {
      errors.push("End date must be today or later");
    }

    // Sunday validation for ALL leave types
    if (startDate && isSunday(startDate)) {
      errors.push("Start date cannot be Sunday");
    }
    if (!isHalfDay && endDate && isSunday(endDate)) {
      errors.push("End date cannot be Sunday");
    }

    // Check Sundays in continuous range for ALL leave types
    if (startDate && endDate && startDate <= endDate && !isHalfDay) {
      const s = new Date(startDate);
      const e = new Date(endDate);
      for (let d = new Date(s); d <= e; d.setDate(d.getDate() + 1)) {
        if (isSunday(d.toISOString().split("T")[0])) {
          errors.push("Leave period cannot include Sundays");
          break;
        }
      }
    }

    // Casual Leave specific rules (max 3 working days)
    if (isCasualLeave() && !isHalfDay) {
      const days = calculateDays();
      if (days > 3) {
        errors.push("Casual Leave maximum 3 continuous working days allowed");
      }
    }

    return errors.length > 0 ? errors : null;
  }, [startDate, endDate, isHalfDay, selectedLeaveType, leaveTypes]);
  ...
  const selectedLeaveStats = useMemo(() => {
    ...
    const effect = selectedLeaveTypeData.leaveEffect;

    if (effect === "DEDUCT") {
      const total = selectedLeaveBalance.totalAvailable ?? 0;
  ...
  // Filtered adjustments with original index
  const filteredAdjustments = useMemo(() => {
    const withIndex = periodAdjustments.map((p, idx) => ({
      ...p,
      originalIndex: idx,
    }));

    if (!isHalfDay) return withIndex;

    return withIndex.filter((p) => {
      const pNum = parseInt(p.period, 10);
  ...
  const loadLeaveTypes = async () => {
    try {
      ...
      console.error("leave types error", err.response?.data || err);
  ...
  const loadYearUsage = async () => {
    try {
      const now = new Date();
      const year = now.getFullYear();
      const monthKey = `${year}-${String(now.getMonth() + 1).padStart(2, "0")}`;
      const res = await axios.get(
        `http://localhost:5000/api/leave-request/faculty/${employeeId}/usage?month=${monthKey}`
      );
      setYearUsage(res.data?.data || []);
    } catch (err) {
      console.error("year usage error", err.response?.data || err);
    }
  };
  ...
  const loadPeriods = async (start, end) => {
    try {
      ...
      // availableSubstitutes (top level) can be kept as a fallback or ignored
      setAvailableSubstitutes(res.data.availableSubstitutes || []);

      const uniqueMap = {};
      fetchedPeriods.forEach((p) => {
        const key = `${p.departmentId}-${p.className}`;
        if (!uniqueMap[key]) {
          uniqueMap[key] = { dept: p.departmentId, class: p.className };
        }
      });
      const uniqueClasses = Object.values(uniqueMap);
      ...
      for (const item of uniqueClasses) {
        try {
          ...
          console.error("subject load error", e.response?.data || e);
        }
      }
      ...
      setPeriodAdjustments(
        fetchedPeriods.map((p) => ({
          ...
          status: "pending",
          // Store availability list for THIS specific period
          periodAvailableSubstitutes: p.availableSubstitutes || []
        }))
      );
    } catch (err) {
      console.error("periods error", err.response?.data || err);
      setPeriods([]);
      setPeriodAdjustments([]);
    } finally {
      setLoading(false);
  ...
  const handleStartDateChange = (e) => {
    ...
    if (newDate && new Date(newDate) <= getYesterday()) {
      setMessage({ type: "error", text: "Start date must be today or later" });
      return;
    }

    if (newDate && isSunday(newDate)) {
      setMessage({ type: "error", text: "Start date cannot be Sunday" });
      return;
    }
  ...
  const handleEndDateChange = (e) => {
    ...
    if (newDate && new Date(newDate) <= getYesterday()) {
      setMessage({ type: "error", text: "End date must be today or later" });
      return;
    }

    if (newDate && isSunday(newDate)) {
      setMessage({ type: "error", text: "End date cannot be Sunday" });
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if ((user.role === "teaching" || user.role === "hod") && !isEmergency) {
      const unadjusted = filteredAdjustments.filter(
        (p) => !p.substituteFacultyId
      );
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto font-sans text-gray-800">
      ...
      {selectedLeaveType && selectedLeaveStats && (
        <div
          className={`mb-8 p-6 border-2 rounded-2xl shadow-xl ${selectedLeaveStats.type === "DEDUCT"
              ...
              <>
                ...
                <div className="text-center p-4 bg-white rounded-xl shadow-md">
                  ...
                  <p className="text-3xl font-black text-blue-600 mt-2">
                    {selectedLeaveStats.remaining}
                  </p>
                  <p className="text-xs text-gray-500">days available</p>
                </div>
                <div
                  className={`text-center p-4 rounded-xl shadow-md ${!selectedLeaveStats.isInsufficient
                ...
                >
                  ...
                  <p
                    className={`text-3xl font-black mt-2 ${!selectedLeaveStats.isInsufficient
                      ? "text-green-600"
                      : "text-red-600"
                      }`}
                  >
                  ...
                  <p
                    className={`text-xs mt-1 ${!selectedLeaveStats.isInsufficient
                      ? "text-green-700"
                ...
                <div className="text-center p-4 bg-white rounded-xl shadow-md md:col-span-2 border-2 border-indigo-100">
                  ...
                  <div className="flex items-center justify-center gap-4 mt-2">
                    ...
                    <span className="text-2xl font-bold text-blue-600">
                      {selectedLeaveStats.requestingDays} (Request)
                    </span>
                    <span className="text-xl text-gray-400">=</span>
                    <span className="text-4xl font-black text-indigo-700">
                      {selectedLeaveStats.willBeTotalUsed}
                    </span>
                  </div>
                  <p className="text-xs text-indigo-600 mt-1 font-medium">
                    Days total after this request
                  </p>
                </div>
              </>
            )}
      ...
      {dateValidation && (
        <div className="mb-6 p-4 bg-red-50 border-2 border-red-200 rounded-xl text-red-800">
          <div className="font-semibold mb-2">Date Validation Errors:</div>
          <ul className="list-disc list-inside space-y-1">
            {dateValidation.map((error, idx) => (
              <li key={idx}>{error}</li>
            ))}
          </ul>
        </div>
      ...
      {leaveBalance.length > 0 && !selectedLeaveType && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {leaveBalance.map((lb, i) => {
            ...
            const total = lb.totalAvailable ?? 0;
            const remaining = lb.remainingLeaves ?? 0;
            const used = lb.usedLeaves ?? 0;

            return (
              <div
                key={i}
                className="p-4 rounded-xl border bg-gray-50 border-gray-100 hover:shadow-md cursor-pointer"
                onClick={() => setSelectedLeaveType(lb.leaveTypeId)}
              >
      ...
      {message.text && (
        <div
          className={`p-4 rounded-xl mb-6 border-2 ${message.type === "error"
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                {isHalfDay && (
                  <div className="flex bg-white rounded border overflow-hidden ml-auto">
                    <button
                      ...
                      className={`px-3 py-1 text-xs ${halfDaySession === "morning"
                    ...
                    <button
                      ...
                      className={`px-3 py-1 text-xs ${halfDaySession === "afternoon"
                        ? "bg-blue-600 text.white"
                        : ""
                        }`}
                    >
          ...
          <div>
            <label className="block text-sm font-semibold mb-2 text-gray-700">
              ...
              <span className="text-xs text-orange-600 ml-1">(No Sundays · Apply before 10 AM on start date)</span>
            </label>
            <input
              type="date"
              value={startDate}
              onChange={handleStartDateChange}
              min={startDate || getTodayLocal()} className="w-full border-2 border-gray-200 rounded-xl p-3 focus:border-blue-400"
              required
            />
          </div>
          {!isHalfDay && (
            <div>
              <label className="block text-sm font-semibold mb-2 text-gray-700">
                ...
                <span className="text-xs text-orange-600 ml-1">
                  (No Sundays{isCasualLeave() ? ", Max 3 working days" : ""})
                </span>
              </label>
              <input
                type="date"
                ...
                min={startDate || getToday().toISOString().split("T")[0]}
                className="w-full border-2 border-gray-200 rounded-xl p-3 focus:border-blue-400"
                required={!isHalfDay}
              />
            </div>
                    ...
                    <tr key={`${p.date}-${p.period}-${idx}`}>
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          onChange={(e) =>
                            updateSubstitute(p.originalIndex, e.target.value)
                          }
                        >
                          <option value="">-- Select Faculty --</option>
                          {(p.periodAvailableSubstitutes || []).map((s, si) => (
                            <option key={si} value={s._id}>
        ...
        <div className="flex justify-between items-center bg-gray-50 p-6 rounded-xl border-2 border-gray-200">
          <div className="text-xl font-bold text-gray-800">
            ...
            {isCasualLeave() && !isHalfDay && calculateDays() > 0 && (
              <span className="text-sm text-orange-600 ml-2">
                {calculateDays() <= 3 ? "✓ Valid duration" : "⚠️ Max 3 days"}
              </span>
            )}
          </div>
          <button
            type="submit"
            disabled={
              loading || !!dateValidation || selectedLeaveStats?.isInsufficient
            }
            className={`px-8 py-3 rounded-xl font-bold text-lg transition-all ${loading || dateValidation || selectedLeaveStats?.isInsufficient
